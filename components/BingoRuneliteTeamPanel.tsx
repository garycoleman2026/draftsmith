...
type Device = {
  id: string; memberId: string; memberName: string; deviceName: string; pluginVersion: string;
  connectionState: 'online' | 'idle' | 'offline' | 'waiting'; lastSeenAt: string;
  lastBoardSeenAt: string | null; lastBatchAt: string | null;
  lastResult: { summary: string; status: string; createdAt: string } | null;
};
type Status = { enabled: boolean; scopes: RuneliteScope[]; rawChatStored: false; onlineDeviceCount: number; devices: Device[] };
...
export function BingoRuneliteTeamPanel({ token, members, onNotice, onError }: {
  ...
  useEffect(() => { const initial = window.setTimeout(() => void load().catch((cause) => onError(cause.message)), 0); const timer = window.setInterval(() => void load().catch(() => undefined), 10_000); return () => { window.clearTimeout(initial); window.clearInterval(timer); }; }, [load, onError]);
  ...
  return (
    <section className="wood-panel p-5">
      ...
      {!status?.enabled ? <p className="mt-3 text-xs leading-relaxed text-[#b8aa87]">The organizer has not enabled RuneLite for this event. Manual claims and screenshots still work.</p> : <>
        ...
        <p className="mt-2 rounded border border-[#83a267]/25 bg-[#22311c] p-2.5 text-[10px] leading-relaxed text-[#c9d894]"><b>Stay logged in:</b> paired XP and supported events sync while you play, usually within about 10–15 seconds. You do not need to log out for Terry’s RuneLite bridge.</p>
        ...
        <div className="mt-4 space-y-2">{members.map((member) => { const devices = status.devices.filter((device) => device.memberId === member.id); return <article className="rounded border border-white/10 bg-black/20 p-3" key={member.id}><div className="flex items-center justify-between gap-2"><div><b className="block text-xs text-[#f2d98f]">{member.name}</b><span className="text-[9px] text-[#9f9272]">{devices.length ? `${devices.filter((device) => device.connectionState === 'online').length} live · ${devices.length} paired` : 'Not paired'}</span></div><button className="scroll-button px-2 py-1.5 text-[9px]" disabled={Boolean(working)} onClick={() => void issue(member.id)}>{working === `pair-${member.id}` ? 'Issuing…' : 'Issue code'}</button></div>{devices.map((device) => <div className="mt-2 border-t border-white/10 pt-2 text-[9px]" key={device.id}><div className="flex items-start justify-between gap-2"><span className="text-[#b8aa87]"><b className={device.connectionState === 'online' ? 'text-[#9ed37f]' : device.connectionState === 'idle' ? 'text-[#e0b85b]' : 'text-[#9f9272]'}>● {connectionLabel(device.connectionState)}</b><br />{device.deviceName} · v{device.pluginVersion}<br />Board checked {relative(device.lastBoardSeenAt)}</span><button className="font-black text-[#e7b296] underline" disabled={Boolean(working)} onClick={() => void revoke(device.id)}>{working === `revoke-${device.id}` ? 'Disconnecting…' : 'Disconnect'}</button></div>{device.lastResult ? <p className="mt-2 rounded bg-black/20 p-2 leading-relaxed text-[#c8bb98]"><b className="text-[#d7ae50]">Latest:</b> {device.lastResult.summary}</p> : <p className="mt-2 text-[#8f8265]">Open RuneLite and press Test connection to check this device.</p>}</div>)}</article>; })}</div>
...
function connectionLabel(value: Device['connectionState']) {
  if (value === 'online') return 'Live';
  if (value === 'idle') return 'Recently seen';
