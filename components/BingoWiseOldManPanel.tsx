...
export function BingoWiseOldManPanel({ data, base, onRefresh, onNotice, onError }: {
  ...
  return (
    <section className="wood-panel p-5">
      ...
      <label className="mt-3 flex items-start gap-2 text-[10px] text-[#c8b990]"><input type="checkbox" checked={autoSync} disabled={!groupId} onChange={(event) => setAutoSync(event.target.checked)} /><span>Run background checkpoints even when this room is closed. Available with a group ID.</span></label>
      ...
      {data.wiseOldMan.autoSync && data.wiseOldMan.nextSyncAt ? <p className="mt-3 rounded border border-white/10 bg-black/20 p-2 text-center text-[10px] text-[#c8b990]">Next background check: {new Date(data.wiseOldMan.nextSyncAt).toLocaleString()}</p> : null}
