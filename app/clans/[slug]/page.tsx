...
export default async function PublicClanPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="realm-bg min-h-screen text-[#eadcb9]">
    ...
    <section className="mx-auto max-w-7xl px-5 pb-20 pt-10 sm:px-8">
      ...
      <section className="mt-10"><div className="flex items-end justify-between border-b border-[#9b792f]/35 pb-4"><div><p className="text-[10px] font-black uppercase tracking-[0.12em] text-[#c69b3c]">Published by this clan</p><h2 className="fantasy-title mt-1 text-3xl font-bold text-[#f5df9b]">Reusable boards</h2></div><Link className="text-xs font-black text-[#e3cf96] underline" href="/templates">Browse every board</Link></div>{data.templates.length ? <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{data.templates.map((template) => <article className="parchment-card p-5 text-[#342817]" key={template.id}><p className="text-[10px] font-black uppercase text-[#80642b]">{template.category}</p><h3 className="fantasy-title mt-2 text-2xl font-bold">{template.name}</h3><p className="mt-2 text-xs leading-relaxed text-[#67583e]">{template.summary}</p><p className="mt-4 text-[10px] font-black uppercase text-[#756748]">{template.cloneCount} uses · ↑ {template.upvoteCount} · ↓ {template.downvoteCount}</p><Link className="scroll-button mt-4 inline-flex px-4 py-2.5 text-xs" href={`/templates/${template.slug}`}>Preview board →</Link></article>)}</div> : <p className="mt-5 text-sm text-[#a99a78]">No community boards published yet.</p>}</section>
    </section>
    <SiteFooter />
  </main>;
}
