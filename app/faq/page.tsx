...
const questions = [
  ...
  ['Can I make and save a custom OSRS bingo without starting an event?', 'Yes. The board studio works without teams, a draft, or an event. Design a 3 × 3 through 7 × 7 board, save it privately under your account, or publish it to the community marketplace so anyone can preview and customize a copy.'],
  ['Which bingo game styles are built in?', 'Classic lines, points, lockout, blackout, category conquest, custom prerequisite progression, and a 7 × 7 center-out expedition are built in. Center-out starts with one middle tile; each completion unlocks only the tile directly above, below, left, or right. It can use a shared team frontier or a separate board per team.'],
  ['How are bingo task times estimated?', 'Straightforward drops use the individual drop rate and efficient individual attempts per hour. XP and quantity tasks use an efficient units-per-hour assumption; execution tasks may use a fixed practice budget. Terry compares total estimated player-hours with the event dates and team size. The estimates are editable planning averages, not RNG guarantees.'],
  ['Do I have to run a draft before making a bingo?', 'No. Use the board studio if you only want to design and save a board. If your clan already has teams, paste each team name and roster into the bingo hall to create the organizer room, private team links, spectator page, Wise Old Man controls, and RuneLite pairing flow.'],
