...
export default async function PresetsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <ContentPage eyebrow="Task library" title={`${OSRS_BINGO_PRESETS.length} tasks ready for your board.`} intro="Find a task, add it to a board, and change anything you need.">
    <form className="wood-panel grid gap-3 p-4 sm:grid-cols-2 xl:grid-cols-[minmax(0,1fr)_190px_170px_auto]" method="get">
    ...
    <section>
      <div className="flex flex-wrap items-end justify-between gap-3"><div><h2>{category === 'All' ? 'Matching tasks' : category}</h2><p className="mt-1 text-sm">{filtered.length} result{filtered.length === 1 ? '' : 's'} · page {page} of {pageCount}</p></div>{(query || category !== 'All' || difficulty !== 'all') ? <Link href="/presets">Clear filters</Link> : null}</div>
      ...
      {pageCount > 1 ? <nav aria-label="Preset pages" className="mt-6 flex items-center justify-center gap-3">{page > 1 ? <Link className="scroll-button px-4 py-2 text-xs no-underline" href={pageHref(page - 1, query, category, difficulty)}>← Previous</Link> : null}<span className="text-sm font-black">{page} / {pageCount}</span>{page < pageCount ? <Link className="scroll-button px-4 py-2 text-xs no-underline" href={pageHref(page + 1, query, category, difficulty)}>Next →</Link> : null}</nav> : null}
    </section>
...
function pageHref(page: number, query: string, category: string, difficulty: 'all' | BingoTaskDifficulty) {
