...
import {
  TEMPLATE_CATEGORIES, listGalleryTemplates, sanitizeGallerySort, type GalleryTemplate,
} from '../../lib/bingo-gallery';

export const metadata: Metadata = {
  title: 'Community OSRS bingo templates — Terry’s Drafting',
  description: 'Search, preview, rate, and reuse OSRS clan bingo boards for points, lockout, progression, blackout, categories, and classic lines.',
...
export default async function TemplateGalleryPage({
  ...
  const templates = await listGalleryTemplates({ query, category, mode, sort });
        ...
        <form className="wood-panel mt-8 grid gap-3 p-4 sm:p-5 lg:grid-cols-[minmax(0,1fr)_180px_180px_170px_auto]" method="get">
          ...
          <label className="text-[10px] font-black uppercase tracking-[0.08em] text-[#c4b48c]">Sort
            <select className="dark-field mt-1 h-11 w-full px-3 text-sm normal-case" defaultValue={sort} name="sort">
              <option value="popular">Most used</option><option value="rating">Top rated</option><option value="newest">Newest</option><option value="name">Name</option>
            </select>
          </label>
...
function TemplateCard({ template }: { template: GalleryTemplate }) {
  return <article className="parchment-card flex h-full flex-col p-5 text-[#342817]">
    ...
    <dl className="mt-5 grid grid-cols-3 gap-2 border-y border-[#8b6a32]/20 py-3 text-center"><div><dt className="text-[8px] font-black uppercase text-[#7d6a47]">Layout</dt><dd className="mt-1 text-xs font-black">{template.gridSize} × {template.gridSize}</dd></div><div><dt className="text-[8px] font-black uppercase text-[#7d6a47]">Uses</dt><dd className="mt-1 text-xs font-black">{template.official ? 'Starter' : template.cloneCount}</dd></div><div><dt className="text-[8px] font-black uppercase text-[#7d6a47]">Rating</dt><dd className="mt-1 text-xs font-black">{template.ratingAverage === null ? 'New' : `${template.ratingAverage.toFixed(1)} ★`}</dd></div></dl>
