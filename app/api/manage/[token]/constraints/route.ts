...
export async function POST(
  ...
  try {
    ...
    const body = (await request.json()) as { type?: unknown; playerAId?: unknown; playerBId?: unknown };
      ...
      db
        .prepare(
          `INSERT INTO draft_constraints
            (id, draft_id, constraint_type, player_a_id, player_b_id, created_at)
           VALUES (?, ?, ?, ?, ?, ?)`,
        )
        .bind(id, draft.id, type, playerAId, playerBId, now),
      db
        .prepare("UPDATE drafts SET result_json = NULL, status = 'collecting', updated_at = ? WHERE id = ?")
        .bind(now, draft.id),
    ]);
    return json({ id, type, playerAId, playerBId }, { status: 201 });
  } catch (error) {
...
function findDraft(token: string) {
  return getDatabase()
    .prepare('SELECT id, team_count, live_started_at FROM drafts WHERE admin_token = ?')
    .bind(token)
    .first<DraftRow>();
}
