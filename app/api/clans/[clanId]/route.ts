...
export async function PATCH(request: Request, context: Context) {
  try {
    ...
    const body = await request.json().catch(() => ({})) as { description?: unknown; publicListing?: unknown };
    ...
    await db.prepare(
      'UPDATE clans SET description = ?, public_listing = ?, updated_at = ? WHERE id = ?',
    ).bind(description, publicListing ? 1 : 0, now, clanId).run();
    await recordAudit(db, {
      clanId, actorUserId: user.id, actorType: 'organizer', eventType: 'clan.profile_updated',
      metadata: { publicListing }, requestId: requestId(request), createdAt: now,
    });
    return json({ saved: true, publicPath: publicListing ? `/clans/${await clanSlug(clanId)}` : null });
  } catch (error) {
